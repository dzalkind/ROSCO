...
class InputWriter_OpenFAST(object):
    ...
    def write_AeroDyn15(self):
        ...
        if 'UAStartRad' in self.fst_vt['AeroDyn15']:
            f.write('{:<22} {:<11} {:}'.format(self.fst_vt['AeroDyn15']['UAStartRad'], 'UAStartRad', '- Starting radius for dynamic stall (fraction of rotor radius) [used only when AFAeroMod=2; if line is missing UAStartRad=0]\n'))
        if 'UAEndRad' in self.fst_vt['AeroDyn15']:
            f.write('{:<22} {:<11} {:}'.format(self.fst_vt['AeroDyn15']['UAEndRad'], 'UAEndRad', '- nding radius for dynamic stall (fraction of rotor radius) [used only when AFAeroMod=2; if line is missing UAEndRad=1]\n'))
