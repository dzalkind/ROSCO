...
public:
    void load(const std::string& filename) {
        ...
        if (!f.is_open()) {
            rosco_throw("ReadControlParameterFileSub", "Cannot open file: %s", filename.c_str());
        }
        std::string line;
    ...
    bool parseDbl(const char* name, double& val, bool allowDefault = true) {
        ...
        if (idx < 0) {
            ...
            rosco_throw("ReadControlParameterFileSub", "Could not find parameter %s in %s", name, filename_.c_str());
        }
        std::istringstream iss(getValuePart(idx));
        if (!(iss >> val)) {
            if (allowDefault) { val = 0.0; return true; }
            rosco_throw("ReadControlParameterFileSub", "Error reading value for %s in %s", name, filename_.c_str());
        }
        return true;
    }
    ...
    bool parseDblParamArray(const char* name, ParamArray& arr, int n, bool allowDefault = true) {
        ...
        for (int i = 0; i < n; i++) {
            if (!(iss >> arr[i])) {
                ...
                rosco_throw("ReadControlParameterFileSub", "Error reading array element %d for %s in %s",
                             i + 1, name, filename_.c_str());
            }
        }
        return true;
    }
    ...
    const char* priPath)
{
    ...
    if (CntrPar.OL_Mode > 0 && !CntrPar.OL_Filename.empty()) {
        ...
        // Print channel list
        std::string olString;
        if (CntrPar.Ind_BldPitch.size() >= 3) {
            if (CntrPar.Ind_BldPitch[0] > 0) olString += " BldPitch1 ";
            if (CntrPar.Ind_BldPitch[1] > 0) olString += " BldPitch2 ";
            if (CntrPar.Ind_BldPitch[2] > 0) olString += " BldPitch3 ";
        }
        ...
        if (!olFile.is_open()) {
            rosco_throw("ReadControlParameterFileSub", "%s does not exist", CntrPar.OL_Filename.c_str());
        }
        ...
        if (nRows < 1) {
            rosco_throw("ReadControlParameterFileSub", "Error: No data lines found in OL input file.");
        }
        ...
        if (CntrPar.Ind_Azimuth > 0) {
            ...
            unwrap(CntrPar.OL_Azimuth.data(), nRows, CntrPar.OL_Azimuth.data());
        }
