...
CONTAINS
    ...
    SUBROUTINE ExtController(avrSWAP, CntrPar, LocalVar, ErrVar)
        ...
        TYPE(ErrorVariables)                    :: ErrVar_Ext
        ...
        IF (LocalVar%iStatus == 0) THEN  ! Only compute control calculations if no error has occurred and we are not on the last time step
            ...
            ALLOCATE(ext_dll_data%avrSWAP(max_avr_entries)) !(1:max_avr_entries)
             ...
             ! Convert types for external DLL
            
            ! avcMSG     = TRANSFER("Test"//C_NULL_CHAR, avcMSG, SIZE(avcMSG))
            PRINT *, avcOUTNAME
        ...
        ! This throws errors about sizing, so use same avrSWAP and skip this
        ! print *, SIZE(avrSWAP)
        ! ALLOCATE(ext_dll_data%avrSWAP(max_avr_entries)) !(1:max_avr_entries)
        ext_dll_data%avrSWAP = avrSWAP(1:max_avr_entries)

        ! ! Fix some length parameters
        ...
        ! Fix some length parameters
        ! avrSWAP(49) = 1024 + 1                      !> * Record 49: Maximum number of characters in the "MESSAGE" argument (-) [size of ExtErrMsg argument plus 1 (we add one for the C NULL CHARACTER)]
        ! avrSWAP(50) = LEN_TRIM(ExtDLL_InFile) +1  !> * Record 50: Number of characters in the "INFILE"  argument (-) [trimmed length of ExtDLL_InFile parameter plus 1 (we add one for the C NULL CHARACTER)]
        ! avrSWAP(51) = LEN_TRIM(ExtRootName)   +1  !> * Record 51: Number of characters in the "OUTNAME" argument (-) [trimmed length of ExtRootName parameter plus 1 (we add one for the C NULL CHARACTER)]
    


        print *, 'DLL_Ext%ProcAddr(1):', DLL_Ext%ProcAddr(1)
        ...
        ! CALL DLL_Legacy_Subroutine (avrSWAP, ErrVar%aviFAIL, accINFILE, avcOUTNAME, avcMSG ) 

        PRINT *, 'here!'
        ! PRINT *, ext_dll_data%avrSWAP(1:85)
    
        PRINT *, ext_dll_data%avrSWAP(42)
        ! PRINT *, avrSWAP(1:85)

        PRINT *, 'here!2'

        ! avrSWAP(1:max_avr_entries) = ext_dll_data%avrSWAP

        PRINT *, "Finished in ExtController"
