...
def _write_cpp_debug(yfile):
    ...
    with open(out_path, 'w') as f:
        ...
        f.write('// .dbg3 (avrSWAP): HDF5 mode writes it as a second dataset ("/avrSWAP")\n')
        f.write('// in the same .RO.h5 file via dbg_writer; text mode keeps the .dbg3 file.\n')
        f.write('static std::ofstream dbg3_file;\n')
        f.write('static bool dbg3_use_hdf5 = false;\n')
        ...
        # .dbg3 init: HDF5 mode -> second dataset in dbg_writer's file; text mode -> .dbg3 file
        ...
        f.write('                }\n')
        f.write('            }\n')
        f.write('            dbg3_use_hdf5 = (fmt == OutputFormat::HDF5) && dbg_writer;\n')
        f.write('            if (dbg3_use_hdf5) {\n')
        ...
        f.write('                dbg_writer->open_avrswap(avr_label_ptrs.data(), (int)avr_indices.size());\n')
        f.write('            } else {\n')
        ...
        f.write('            std::vector<double> avr_vals(avr_indices.size());\n')
        f.write('            for (size_t i = 0; i < avr_indices.size(); i++) {\n')
        f.write('                avr_vals[i] = clamp_debug((double)avrSWAP[avr_indices[i] - 1]);\n')
        f.write('            }\n')
        f.write('            if (dbg3_use_hdf5 && dbg_writer) {\n')
        f.write('                dbg_writer->write_avrswap_row(avr_vals.data(), (int)avr_vals.size());\n')
        f.write('            } else {\n')
        f.write('                char buf[32];\n')
        f.write('                snprintf(buf, sizeof(buf), "%20.5f", LocalVar.Time);\n')
        f.write('                dbg3_file << buf;\n')
        f.write('                for (size_t i = 0; i < avr_vals.size(); i++) {\n')
        f.write('                    dbg3_file << "     ";\n')
        f.write('                    snprintf(buf, sizeof(buf), "%20.5E", avr_vals[i]);\n')
        f.write('                    dbg3_file << buf;\n')
        f.write('                }\n')
        f.write('                dbg3_file << "\\n";\n')
        f.write('            }\n')
