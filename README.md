...
> **This is the `c++` branch.** The controller is now pure C++ — the Fortran source has been
> removed entirely. It was translated from Fortran with [VIT](https://github.com/NatLabRockies/vit)
> and subsequently refactored; see [REFACTOR_NOTES.md](REFACTOR_NOTES.md) for what changed and
> why. Intended for a future v3.0 release; `master` is unchanged for existing users.
>
> The controller parameter struct, TOML/`DISCON.IN` parsers, and debug output are generated
> from `rosco/controller/rosco_registry/rosco_types.yaml` — edit the YAML and re-run
