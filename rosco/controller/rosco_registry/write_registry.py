...
def generate(yfile):
    '''
    Generates full registry and ROSCO I/O files (C++)
    '''
    generate_cpp(yfile)
    ...
    with open(out_path, 'w') as f:
        ...
        for name, param in params.items():
            ...
            if alloc:
                ...
                eq = param.get('equals')
                f.write(f'    {name} = tbl["{name}"].value_or({eq if eq is not None else 0.0});\n')
            elif cpp_type == 'int':
                eq = param.get('equals')
                f.write(f'    {name} = (int)tbl["{name}"].value_or((int64_t){eq if eq is not None else 0});\n')
            elif cpp_type == 'float':
                eq = param.get('equals')
                f.write(f'    {name} = (float)tbl["{name}"].value_or({eq if eq is not None else 0.0});\n')
...
def _write_cpp_debug(yfile):
    ...
    with open(out_path, 'w') as f:
        ...
        f.write('// .dbg3 (avrSWAP): text file in Text mode, "/avrSWAP" dataset in dbg_writer\'s\n')
        f.write('// HDF5 file (same file as .dbg) in HDF5 mode.\n')
        f.write('static std::ofstream dbg3_file;\n')
        f.write('static std::vector<int32_t> avr_indices;\n')
        f.write('static OutputFormat dbg3_fmt = OutputFormat::Text;\n')
        f.write('\n')
        ...
        # .dbg3 init: HDF5 mode opens "/avrSWAP" dataset in dbg_writer's file;
        # text mode opens the standalone .dbg3 text file (unchanged format).
        ...
        f.write('                }\n')
        f.write('            }\n')
        f.write('            dbg3_fmt = fmt;\n')
        f.write('            if (fmt == OutputFormat::HDF5 && dbg_writer) {\n')
        f.write('                std::vector<std::string> avr_labels;\n')
        f.write('                std::vector<const char*> avr_label_ptrs;\n')
        f.write('                avr_labels.reserve(avr_indices.size());\n')
        f.write('                for (size_t i = 0; i < avr_indices.size(); i++) {\n')
        f.write('                    avr_labels.push_back("AvrSWAP(" + std::to_string(avr_indices[i]) + ")");\n')
        f.write('                }\n')
        f.write('                for (auto& s : avr_labels) avr_label_ptrs.push_back(s.c_str());\n')
        f.write('                dbg_writer->open_avrswap(avr_label_ptrs.data(), (int)avr_label_ptrs.size());\n')
        f.write('            } else {\n')
        ...
        f.write('            if (dbg3_fmt == OutputFormat::HDF5 && dbg_writer) {\n')
        f.write('                std::vector<double> avr_row;\n')
        f.write('                avr_row.reserve(avr_indices.size());\n')
        f.write('                for (size_t i = 0; i < avr_indices.size(); i++) {\n')
        f.write('                    avr_row.push_back(clamp_debug((double)avrSWAP[avr_indices[i] - 1]));\n')
        f.write('                }\n')
        f.write('                dbg_writer->write_avrswap_row(avr_row.data(), (int)avr_row.size());\n')
        f.write('            } else {\n')
        f.write('                char buf[32];\n')
